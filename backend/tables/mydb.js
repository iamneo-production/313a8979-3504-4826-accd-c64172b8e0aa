...
con.connect(function(err) {
    ...
    // });
    var sql = "create table expenseModel(expenseId varchar(256),billNumber int,billImage blob,billCost int,datedOn date,status varchar(256),remark varchar(256),claimedBy varchar(256),Primary key (expenseId))";
    con.query(sql, function (err, result) {
        if (err) throw err;
        console.log("table created");
      });
